...
WeatherFunction.prototype={
	...
	getAttributesForAll: function(){
		...
		for(var state in this.stateMap){
			...
			attributeArray.push({state:state,temp:obj.current_observation.temp_c,wind:obj.current_observation.wind_mph,humidity:obj.current_observation.relative_humidity});
			i=i+1;
		}
	...
	getTestData : function(){
		...
		for(var state in this.stateMap){
			...
			for(i=1; i<=12; i++){
				var fileName = "./history/city" + j + "_Month" + i + ".txt";
				fs.writeFile(fileName, this.getJSON("http://www.wunderground.com/history/airport/" + this.airPortMap[state] + "/2014/" + i + "/1/MonthlyHistory.html?req_city=" + this.stateMap[state] + "&req_state=" + state + "&reqdb.magic=1&reqdb.wmo=99999&format=1"));
			}
			j = j+1;
	...
	getHistoricDataForState : function(){
		var array = fs.readFileSync("./history/mod/mod_city49.txt").toString().split("\n");
		var labels = array.shift();
		fs.writeFile("new.txt", labels);
		...
		for(i in array){
			...
			if(splitArray[1] != " " && splitArray[2] != " " && splitArray[3] != " " && splitArray[1] != null ){
				...
				fs.appendFileSync('new.txt', ''+output+'\n');					
			}
